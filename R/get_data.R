...
#' @param seperator The separator to be used to distinguish between values
#'
#' @importFrom gtools mixedsort
#' @examples get_data("Documents/folder/", save=TRUE , "Documents/folder/data_out/")
#'
#' @export
get_data <- function(path, save=FALSE , path_out, seperator=",") {
  ...
  for (i in 1:len) {
    ...
    slice_list[[i]] <- data.matrix(read.csv(file_path, row.names = 1, sep=seperator))
  }
